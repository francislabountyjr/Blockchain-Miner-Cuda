...
{                                             \
    *(x) =   ((uint32) *((str) + 3)      )    \
    ...
    __device__ void sha256(unsigned char* input, unsigned char random[], unsigned int length, unsigned int randomLen, unsigned char digest[])
    {
        SHA256 ctx = SHA256();
        ctx.init();
        ctx.update(input, length);
        ctx.update(random, randomLen);
        ctx.final(digest);
    }
}